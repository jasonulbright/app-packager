...
.DESCRIPTION
    ...
    plain x64 machine-wide MSI is taken.
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\VSCodium\VSCodium\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\VSCodium).
    Default: C:\temp\ap
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/VSCodium/vscodium/releases/latest"

$VendorFolder = "VSCodium"
$AppFolder    = "VSCodium"

$BaseDownloadRoot = Join-Path $DownloadRoot "VSCodium"
...
function Get-LatestVSCodiumRelease {
    param([switch]$Quiet)
    ...
    try {
        ...
        # Anchored so the updates-disabled MSI and the .sha1/.sha256 sidecars
        # that share the prefix are not taken.
        $rx = '^VSCodium-x64-(?<ver>\d+(?:\.\d+)+)\.msi$'
        ...
        Write-Log "Latest VSCodium version      : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get VSCodium version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageVSCodium {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "VSCodium (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Get version ---
    $releaseInfo = Get-LatestVSCodiumRelease
    if (-not $releaseInfo) { throw "Could not resolve VSCodium version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "VSCodium" }
...
function Invoke-PackageVSCodium {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "VSCodium (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestVSCodiumRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("VSCodium GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "VSCodium (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageVSCodium
    }
    elseif ($PackageOnly) {
        Invoke-PackageVSCodium
    }
    else {
        Invoke-StageVSCodium
        Invoke-PackageVSCodium
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-vscodium'
