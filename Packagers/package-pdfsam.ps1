...
.SYNOPSIS
    Packages PDFsam Basic (x64) MSI for MECM.

.DESCRIPTION
    Resolves the latest PDFsam release from the GitHub releases API, downloads
    the windows-x64 MSI, stages content to a versioned local folder with ARP
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available PDFsam Basic version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/torakiki/pdfsam/releases/latest"

$VendorFolder = "Sober Lemur"
$AppFolder    = "PDFsam Basic"

$BaseDownloadRoot = Join-Path $DownloadRoot "PDFsam Basic"
...
function Get-LatestPdfsamRelease {
    <#
    .SYNOPSIS
        Returns the newest PDFsam Basic version and its windows-x64 MSI URL.
    .DESCRIPTION
        Every release also ships Linux, macOS and zip assets plus detached .asc
        signatures, so the asset match is pinned to the windows-x64 MSI and
        excludes the signature file.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $asset = $release.assets |
            Where-Object { $_.name -match '^pdfsam-basic-.*-windows-x64\.msi$' } |
            Select-Object -First 1
        if (-not $asset) { throw "No windows-x64 MSI asset found in the latest release." }

        Write-Log "Latest PDFsam Basic version  : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get PDFsam Basic version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StagePdfsam {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "PDFsam Basic (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestPdfsamRelease
    if (-not $releaseInfo) { throw "Could not resolve PDFsam Basic version." }
    ...
    # The release tag is three-part (6.0.5) while the MSI carries a four-part
    # ProductVersion (6.0.5.0); the registry receives the MSI form.
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "Sober Lemur S.r.l." }
...
function Invoke-PackagePdfsam {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "PDFsam Basic (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestPdfsamRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("PDFsam Basic GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "PDFsam Basic (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-pdfsam'
