...
App: gsudo
CMName: gsudo
VendorUrl: https://gerardog.github.io/gsudo/
...
.SYNOPSIS
    Packages gsudo (x64) MSI for MECM.

.DESCRIPTION
    Resolves the latest gsudo release from the GitHub releases API, downloads
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\gerardog\gsudo\<Version>
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available gsudo version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/gerardog/gsudo/releases/latest"

$VendorFolder = "gerardog"
$AppFolder    = "gsudo"

$BaseDownloadRoot = Join-Path $DownloadRoot "gsudo"
...
function Get-LatestGsudoRelease {
    <#
    .SYNOPSIS
        Returns the newest gsudo release version and its x64 MSI asset.
    .DESCRIPTION
        The release carries x86, x64 and ARM64 MSIs plus a portable zip under
        fixed asset names; the asset name is matched on the x64 MSI so an x86 or
        ARM64 payload is never staged as the x64 deployment type. The release tag
        is used only for logging; the staged version comes from the MSI itself.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if ($release.prerelease) { throw "Latest gsudo release is flagged prerelease; refusing to stage." }
        ...
        $asset = $release.assets |
            Where-Object { $_.name -match '(?i)^gsudo\.setup\.x64\.msi$' } |
            Select-Object -First 1
        if (-not $asset) { throw "No x64 MSI asset found in the latest release." }

        Write-Log "Latest gsudo release tag   : $tagVersion" -Quiet:$Quiet
        ...
        Write-Log "Failed to get gsudo version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageGsudo {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "gsudo (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestGsudoRelease
    if (-not $releaseInfo) { throw "Could not resolve gsudo version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "gerardog" }
...
function Invoke-PackageGsudo {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "gsudo (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestGsudoRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("gsudo GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "gsudo (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-gsudo'
