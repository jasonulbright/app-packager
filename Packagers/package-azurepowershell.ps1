<#
Vendor: Microsoft
App: Azure PowerShell (Az module, x64)
CMName: Azure PowerShell
VendorUrl: https://learn.microsoft.com/powershell/azure/
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Microsoft\Azure PowerShell\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\AzurePowerShell).
    Default: C:\temp\ap
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/Azure/azure-powershell/releases/latest"

$VendorFolder = "Microsoft"
$AppFolder    = "Azure PowerShell"

$BaseDownloadRoot = Join-Path $DownloadRoot "AzurePowerShell"
...
function Get-LatestAzurePowerShellRelease {
    <#
    .SYNOPSIS
        Returns the latest Azure PowerShell version and its x64 MSI asset URL.
    .DESCRIPTION
        The release tag carries a marketing month suffix (v16.3.0-September2026)
        and the four-part build number lives only in the asset file name, so the
        version comes from the asset. The x64 suffix is matched explicitly
        because the release also carries an x86 MSI.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $asset = $release.assets |
            Where-Object { $_.name -match '(?i)^Az-Cmdlets-(\d+(\.\d+)+)-x64\.msi$' } |
            Select-Object -First 1
        if (-not $asset) { throw "No Az-Cmdlets x64 MSI asset found in the latest release." }

        $m = [regex]::Match($asset.name, '(?i)^Az-Cmdlets-(?<ver>\d+(\.\d+)+)-x64\.msi$')
        $version = $m.Groups['ver'].Value

        Write-Log "Latest Azure PowerShell ver  : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get Azure PowerShell version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageAzurePowerShell {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Azure PowerShell (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestAzurePowerShellRelease
    if (-not $releaseInfo) { throw "Could not resolve Azure PowerShell version." }
    ...
    $appName = $productName
    if ([string]::IsNullOrWhiteSpace($appName)) { $appName = "Azure PowerShell" }
...
function Invoke-PackageAzurePowerShell {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Azure PowerShell (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestAzurePowerShellRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Azure PowerShell GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Azure PowerShell (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageAzurePowerShell
    }
    elseif ($PackageOnly) {
        Invoke-PackageAzurePowerShell
    }
    else {
        Invoke-StageAzurePowerShell
        Invoke-PackageAzurePowerShell
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-azurepowershell'
