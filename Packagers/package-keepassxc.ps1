...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\KeePassXC\KeePassXC\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\KeePassXC).
    Default: C:\temp\ap
...
$VendorFolder = "KeePassXC"
$AppFolder    = "KeePassXC"

$BaseDownloadRoot = Join-Path $DownloadRoot "KeePassXC"
...
function Get-LatestKeePassXCRelease {
    <#
    .SYNOPSIS
        Returns the newest KeePassXC version and its Win64 MSI URL.
    .DESCRIPTION
        The release also carries a LegacyWindows MSI built for pre-AVX2 CPUs
        and a matching zip, so the asset name is anchored on the plain Win64
        MSI rather than the first Windows asset found.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $asset = $release.assets |
            Where-Object { $_.name -match '^KeePassXC-(\d+(?:\.\d+)+)-Win64\.msi$' } |
            Select-Object -First 1
        if (-not $asset) { throw "Could not locate a Win64 MSI asset in the latest release." }

        $version = [regex]::Match($asset.name, '^KeePassXC-(?<ver>\d+(?:\.\d+)+)-Win64\.msi$').Groups['ver'].Value

        Write-Log "Latest KeePassXC version     : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get KeePassXC version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageKeePassXC {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "KeePassXC (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestKeePassXCRelease
    if (-not $releaseInfo) { throw "Could not resolve KeePassXC version." }
    ...
    # requires regardless of the invoking context.
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "KeePassXC Team" }
...
function Invoke-PackageKeePassXC {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "KeePassXC (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestKeePassXCRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("KeePassXC GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "KeePassXC (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageKeePassXC
    }
    elseif ($PackageOnly) {
        Invoke-PackageKeePassXC
    }
    else {
        Invoke-StageKeePassXC
        Invoke-PackageKeePassXC
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-keepassxc'
