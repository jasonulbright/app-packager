...
.SYNOPSIS
    Packages M365 Visio (x64) for ConfigMgr using the Office Deployment Tool.
...
function Invoke-StageM365Visio {
    ...
    if ($isOnline) {
        ...
        $appName = "M365 Visio - $version (x64) ($($ch.Tag))"
    }
...
try {
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-m365visio-x64'
