<#
Vendor: Azul
App: Azul Zulu JDK 11 (x64)
CMName: Azul Zulu JDK 11 (x64)
...
DownloadPageUrl: https://www.azul.com/downloads/?version=java-11-lts&os=windows&architecture=x86-64-bit&package=jdk
...
$VendorFolder   = "Azul"
$AppFolder      = "Zulu JDK 11 (x64)"
$Publisher      = "Azul Systems"
$BaseDownloadRoot = Join-Path $DownloadRoot "ZuluJDK11"

$DisplayTitle   = "Azul Zulu JDK 11 (x64)"
...
function Get-LatestZuluRelease {
    <#
    .SYNOPSIS
        Returns the newest GA Zulu 11 jdk x64 Windows MSI for this feature version.
    .DESCRIPTION
    ...
    try {
        ...
        if ($candidates.Count -lt 1) {
            throw "No x64 Windows jdk MSI package found for Zulu 11."
        }
        ...
        Write-Log "Failed to get Zulu 11 jdk version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageZulu {
    ...
    if (-not $releaseInfo) { throw "Could not resolve Zulu 11 jdk version." }
...
if ($GetLatestVersionOnly) {
    try {
        ...
        [Console]::Error.WriteLine("Zulu JDK 11 GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-zulu-jdk11'
