<#
Vendor: MSEndpointMgr
App: Intune Debug Toolkit
CMName: Intune Debug Toolkit
VendorUrl: https://msendpointmgr.com/
...
.DESCRIPTION
    ...
    of the default branch and carries its version in the file name, so the
    contents API is the version source.
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available Intune Debug Toolkit version string and exits.
...
# --- Configuration ---
$GitHubContentsUrl = "https://api.github.com/repos/MSEndpointMgr/IntuneDebugToolkit/contents/"

$VendorFolder = "MSEndpointMgr"
$AppFolder    = "Intune Debug Toolkit"

$BaseDownloadRoot = Join-Path $DownloadRoot "Intune Debug Toolkit"
...
function Assert-MsiPayload {
    ...
    .DESCRIPTION
        A raw-content redirect that lands on an error page answers 200 with
...
function Get-LatestIntuneDebugToolkitRelease {
    <#
    .SYNOPSIS
        Returns the newest toolkit version and its MSI download URL.
    .DESCRIPTION
        Older MSIs are replaced rather than accumulated, but the root listing is
        still sorted as versions so a branch that carries more than one shipped
        MSI resolves to the highest.
    #>
    param([switch]$Quiet)

    Write-Log "GitHub contents API          : $GitHubContentsUrl" -Quiet:$Quiet
    ...
    try {
        ...
        $rxMatches = $rx.Matches($json)
        if ($rxMatches.Count -lt 1) { throw "Could not locate a shipped MSI in the repository root." }
        ...
        $best = $rxMatches |
            ForEach-Object {
                [pscustomobject]@{
                    ...
                    DownloadUrl = $_.Groups['url'].Value
                }
            } |
            Sort-Object { [version]$_.Version } |
            Select-Object -Last 1

        Write-Log "Latest toolkit version       : $($best.Version)" -Quiet:$Quiet
        return $best
    }
    catch {
        Write-Log "Failed to get Intune Debug Toolkit version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageIntuneDebugToolkit {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Intune Debug Toolkit (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestIntuneDebugToolkitRelease
    if (-not $releaseInfo) { throw "Could not resolve Intune Debug Toolkit version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "MSEndpointMgr" }

    $appName = $productName
    if ([string]::IsNullOrWhiteSpace($appName)) { $appName = "Intune Debug Toolkit" }
...
function Invoke-PackageIntuneDebugToolkit {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Intune Debug Toolkit (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestIntuneDebugToolkitRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Intune Debug Toolkit GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Intune Debug Toolkit (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-Log "GitHubContentsUrl            : $GitHubContentsUrl"
    Write-Log ""

    if ($StageOnly) {
        Invoke-StageIntuneDebugToolkit
    }
    elseif ($PackageOnly) {
        Invoke-PackageIntuneDebugToolkit
    }
    else {
        Invoke-StageIntuneDebugToolkit
        Invoke-PackageIntuneDebugToolkit
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-intunedebugtoolkit'
