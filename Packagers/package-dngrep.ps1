...
App: dnGREP
CMName: dnGREP
VendorUrl: https://dngrep.github.io/
...
.SYNOPSIS
    Packages dnGREP (x64) MSI for MECM.

.DESCRIPTION
    Resolves the latest dnGREP release from the GitHub releases API, downloads
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\dnGREP\dnGREP\<Version>
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available dnGREP version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/dnGrep/dnGrep/releases/latest"

$VendorFolder = "dnGREP"
$AppFolder    = "dnGREP"

$BaseDownloadRoot = Join-Path $DownloadRoot "dnGREP"
...
function Get-LatestDnGrepRelease {
    <#
    .SYNOPSIS
        Returns the newest dnGREP release version and its x64 MSI asset.
    .DESCRIPTION
        The release carries x86, x64 and ARM64 MSIs plus portable zips; the
        asset name is matched on the x64 MSI so an ARM64 or x86 payload is
        never staged as the x64 deployment type. The release tag carries four
        version fields while the MSI name carries three, so the tag is used
        only for logging and the staged version comes from the MSI itself.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if ($release.prerelease) { throw "Latest dnGREP release is flagged prerelease; refusing to stage." }
        ...
        $asset = $release.assets |
            Where-Object { $_.name -match '(?i)^dnGREP\..*\.x64\.msi$' } |
            Select-Object -First 1
        if (-not $asset) { throw "No x64 MSI asset found in the latest release." }

        Write-Log "Latest dnGREP release tag    : $tagVersion" -Quiet:$Quiet
        ...
        Write-Log "Failed to get dnGREP version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageDnGrep {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "dnGREP (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestDnGrepRelease
    if (-not $releaseInfo) { throw "Could not resolve dnGREP version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "dnGREP" }
...
function Invoke-PackageDnGrep {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "dnGREP (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestDnGrepRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("dnGREP GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "dnGREP (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-dngrep'
