<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 21 (x64, EXE)
CMName: Microsoft OpenJDK 21 (EXE)
...
$AppFolder        = "OpenJDK 21 (EXE)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 21 (EXE)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK21-EXE"
...
function Invoke-StageMsOpenJdk21Exe {
    Write-Log ""
    Write-Log ("=" * 60)
    ...
    $uninstallContent = @'
$key = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{11F8EC6C-68A4-9099-96F9-E81566D319CC}_is1'
...
function Invoke-PackageMsOpenJdk21Exe {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk21Exe
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk21Exe
    }
    else {
        Invoke-StageMsOpenJdk21Exe
        Invoke-PackageMsOpenJdk21Exe
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk21-exe'
