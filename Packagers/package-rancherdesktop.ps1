...
Vendor: SUSE
App: Rancher Desktop
CMName: Rancher Desktop
VendorUrl: https://rancherdesktop.io/
...
.SYNOPSIS
    Packages Rancher Desktop (x64) MSI for MECM.

.DESCRIPTION
    Resolves the latest Rancher Desktop release from the GitHub releases API,
    downloads the per-machine Setup MSI, stages content to a versioned local
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available Rancher Desktop version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/rancher-sandbox/rancher-desktop/releases/latest"

$VendorFolder = "SUSE"
$AppFolder    = "Rancher Desktop"

$BaseDownloadRoot = Join-Path $DownloadRoot "Rancher Desktop"
...
function Get-LatestRancherDesktopRelease {
    <#
    .SYNOPSIS
        Returns the latest Rancher Desktop version and its Setup MSI URL.
    .DESCRIPTION
        The release carries installers for three platforms; only the Windows
        Setup MSI is per-machine and silent-installable. Checksum assets share
        the MSI name with a suffix, so the match is anchored to the extension.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $asset = $release.assets |
            Where-Object { $_.name -match '^Rancher\.Desktop\.Setup\..+\.msi$' } |
            Select-Object -First 1
        if (-not $asset) { throw "No Setup MSI asset found in the latest release." }

        Write-Log "Latest Rancher Desktop ver   : $version" -Quiet:$Quiet
        return @{
            Version     = $version
            FileName    = $asset.name
        ...
        Write-Log "Failed to get Rancher Desktop version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageRancherDesktop {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Rancher Desktop (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestRancherDesktopRelease
    if (-not $releaseInfo) { throw "Could not resolve Rancher Desktop version." }
    ...
    if (-not (Test-Path -LiteralPath $localMsi)) {
        Write-Log "Downloading MSI (multi-hundred-MB payload)..."
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "SUSE" }
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        ...
        RunningProcess  = @("Rancher Desktop")
        Detection       = @{
...
function Invoke-PackageRancherDesktop {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Rancher Desktop (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestRancherDesktopRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Rancher Desktop GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Rancher Desktop (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageRancherDesktop
    }
    elseif ($PackageOnly) {
        Invoke-PackageRancherDesktop
    }
    else {
        Invoke-StageRancherDesktop
        Invoke-PackageRancherDesktop
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-rancherdesktop'
