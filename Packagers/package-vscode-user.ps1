...
$AppFolder    = "Visual Studio Code (User)"
$BaseDownloadRoot = Join-Path $DownloadRoot "VSCodeUser"
...
function New-VSCodeUserInstallWrapper {
...
function New-VSCodeUserUninstallWrapper {
    return (
        '$uninstall = Join-Path $env:LOCALAPPDATA ''Programs\Microsoft VS Code\unins000.exe''',
...
function Invoke-StageVSCodeUser {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Visual Studio Code (User) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    $installerFileName = "VSCodeUserSetup-x64-$version.exe"
    ...
    Write-ContentWrappers -OutputPath $localContentPath `
        -InstallPs1Content (New-VSCodeUserInstallWrapper -InstallerFile $installerFileName) `
        -UninstallPs1Content (New-VSCodeUserUninstallWrapper)

    $manifestPath = Join-Path $localContentPath "stage-manifest.json"
    Write-StageManifest -Path $manifestPath -ManifestData @{
        AppName                  = "Visual Studio Code (User) - $version"
        ...
        UninstallCommand         = "%LOCALAPPDATA%\Programs\Microsoft VS Code\unins000.exe"
...
function Invoke-PackageVSCodeUser {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Visual Studio Code (User) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageVSCodeUser
    }
    elseif ($PackageOnly) {
        Invoke-PackageVSCodeUser
    }
    else {
        Invoke-StageVSCodeUser
        Invoke-PackageVSCodeUser
