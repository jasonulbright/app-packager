<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 25 (x64, MSI)
CMName: Microsoft OpenJDK 25 (MSI)
...
.SYNOPSIS
    Packages Microsoft Build of OpenJDK 25 (x64) MSI, per machine, for ConfigMgr.
...
$AppFolder        = "OpenJDK 25 (MSI)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 25 (MSI)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK25-MSI"
...
function Invoke-StageMsOpenJdk25Msi {
    Write-Log ""
    Write-Log ("=" * 60)
...
function Invoke-PackageMsOpenJdk25Msi {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk25Msi
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk25Msi
    }
    else {
        Invoke-StageMsOpenJdk25Msi
        Invoke-PackageMsOpenJdk25Msi
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk25-msi'
