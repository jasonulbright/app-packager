...
App: Box Drive
CMName: Box Drive
VendorUrl: https://www.box.com/resources/downloads
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Box\Box Drive\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\BoxDrive).
    Default: C:\temp\ap
...
# --- Configuration ---
$MsiDownloadUrl = "https://e3.boxcdn.net/box-installers/desktop/releases/win/Box-x64.msi"
$MsiFileName    = "Box-x64.msi"

$VendorFolder = "Box"
$AppFolder    = "Box Drive"

$BaseDownloadRoot = Join-Path $DownloadRoot "BoxDrive"
...
function Assert-MsiPayload {
    ...
    .DESCRIPTION
        The CDN answers 200 with an HTML body for a retired path, which would
...
function Get-BoxDriveMsiProperties {
    <#
    .SYNOPSIS
        Downloads the current Box Drive MSI and returns its property map.
    #>
    param([switch]$Quiet)

    Write-Log "Box Drive MSI URL            : $MsiDownloadUrl" -Quiet:$Quiet
    ...
    if (-not $props -or [string]::IsNullOrWhiteSpace($props["ProductVersion"])) {
        throw "Cannot read ProductVersion from the Box Drive MSI."
    }
...
function Invoke-StageBoxDrive {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Box Drive (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Download ---
    Write-Log "Downloading Box Drive MSI..."
    $download = Get-BoxDriveMsiProperties

    $localMsi = $download.LocalPath
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "Box, Inc." }
...
function Invoke-PackageBoxDrive {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Box Drive (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $download = Get-BoxDriveMsiProperties -Quiet
        Write-Output $download.Properties["ProductVersion"]
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Box Drive GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Box Drive (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageBoxDrive
    }
    elseif ($PackageOnly) {
        Invoke-PackageBoxDrive
    }
    else {
        Invoke-StageBoxDrive
        Invoke-PackageBoxDrive
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-boxdrive'
