...
.SYNOPSIS
    Packages Azul Zulu JDK 17 (x64) MSI for MECM.
...
$VendorFolder   = "Azul"
$AppFolder      = "Zulu JDK 17 (x64)"
$Publisher      = "Azul Systems"
$BaseDownloadRoot = Join-Path $DownloadRoot "ZuluJDK17"

$DisplayTitle   = "Azul Zulu JDK 17 (x64)"
...
function Get-LatestZuluRelease {
    <#
    .SYNOPSIS
        Returns the newest GA Zulu 17 jdk x64 Windows MSI for this feature version.
    .DESCRIPTION
    ...
    try {
        ...
        if ($candidates.Count -lt 1) {
            throw "No x64 Windows jdk MSI package found for Zulu 17."
        }
        ...
        Write-Log "Failed to get Zulu 17 jdk version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageZulu {
    ...
    if (-not $releaseInfo) { throw "Could not resolve Zulu 17 jdk version." }
...
if ($GetLatestVersionOnly) {
    try {
        ...
        [Console]::Error.WriteLine("Zulu JDK 17 GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-zulu-jdk17'
