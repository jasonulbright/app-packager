<#
Vendor: Purslane Tech Pte. Ltd.
App: RustDesk
CMName: RustDesk
VendorUrl: https://rustdesk.com/
...
.DESCRIPTION
    ...
    security tooling in many environments blocks or alerts on remote-access
    software of this class, and some organizations restrict it by policy.
    Confirm the target scope is approved and that any relevant allow-listing
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available RustDesk version string and exits.
...
$VendorFolder = "RustDesk"
$AppFolder    = "RustDesk"

$BaseDownloadRoot = Join-Path $DownloadRoot "RustDesk"
...
function Get-LatestRustDeskRelease {
    <#
    .SYNOPSIS
        Returns the newest RustDesk release version and its x86_64 MSI asset.
    .DESCRIPTION
        The release carries an MSI and an EXE for the same architecture plus
        aarch64 and legacy sciter builds; the asset filter pins the x86_64 MSI
        so an ARM or sciter asset can never be selected by ordering alone.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if (-not $asset) { throw "No x86_64 MSI asset found in release $version." }

        Write-Log "Latest RustDesk version      : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get RustDesk version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageRustDesk {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "RustDesk (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if (-not $releaseInfo) { throw "Could not resolve RustDesk version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "Purslane Tech Pte. Ltd." }
...
function Invoke-PackageRustDesk {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "RustDesk (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestRustDeskRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("RustDesk GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "RustDesk (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageRustDesk
    }
    elseif ($PackageOnly) {
        Invoke-PackageRustDesk
    }
    else {
        Invoke-StageRustDesk
        Invoke-PackageRustDesk
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-rustdesk'
