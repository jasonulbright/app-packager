<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 25 (x64, EXE, per user)
CMName: Microsoft OpenJDK 25 (EXE, per user)
...
.SYNOPSIS
    Packages Microsoft Build of OpenJDK 25 (x64) EXE, per user, for ConfigMgr.
...
$AppFolder        = "OpenJDK 25 (EXE, per user)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 25 (EXE, per user)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK25-EXE-User"
...
function Invoke-StageMsOpenJdk25ExeUser {
    Write-Log ""
    Write-Log ("=" * 60)
    ...
    $uninstallContent = @'
$key = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{9DA4BE12-C58B-3DE4-C047-AF46365065FC}_is1'
...
function Invoke-PackageMsOpenJdk25ExeUser {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk25ExeUser
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk25ExeUser
    }
    else {
        Invoke-StageMsOpenJdk25ExeUser
        Invoke-PackageMsOpenJdk25ExeUser
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk25-exe-user'
