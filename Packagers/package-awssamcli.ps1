...
App: AWS SAM CLI
CMName: AWS SAM CLI
VendorUrl: https://aws.amazon.com/serverless/sam/
...
.SYNOPSIS
    Packages the AWS SAM command line interface (x64) MSI for MECM.

.DESCRIPTION
    Resolves the latest AWS SAM CLI release from the GitHub releases API,
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Amazon Web Services\AWS SAM CLI\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\AWS SAM CLI).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available AWS SAM CLI version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/aws/aws-sam-cli/releases/latest"

$VendorFolder = "Amazon Web Services"
$AppFolder    = "AWS SAM CLI"

$BaseDownloadRoot = Join-Path $DownloadRoot "AWS SAM CLI"
$MsiFileName      = "AWS_SAM_CLI_64_PY3.msi"

# --- Functions ---


function Assert-SamCliPayloadIsMsi {
    <#
    .SYNOPSIS
...
function Get-LatestSamCliRelease {
    <#
    .SYNOPSIS
        Returns Version and DownloadUrl for the latest 64-bit AWS SAM CLI MSI.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $asset = $release.assets |
            Where-Object { $_.name -eq $MsiFileName } |
            Select-Object -First 1
        if (-not $asset) { throw "Asset $MsiFileName not found in release $version." }

        Write-Log "Latest AWS SAM CLI version   : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get AWS SAM CLI release: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageSamCli {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "AWS SAM CLI (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestSamCliRelease
    if (-not $releaseInfo) { throw "Could not resolve AWS SAM CLI release." }
    ...
    # --- Download ---
    # The asset name is constant across releases, so a cached file from an
    # earlier release would shadow the new one.
    $localMsi = Join-Path $BaseDownloadRoot $MsiFileName
    Write-Log "Local MSI path               : $localMsi"
    Write-Log "Downloading AWS SAM CLI MSI..."
    Invoke-DownloadWithRetry -Url $releaseInfo.DownloadUrl -OutFile $localMsi

    Assert-SamCliPayloadIsMsi -Path $localMsi
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        AppName         = "AWS SAM CLI $productVersionRaw"
        Publisher       = $publisher
...
function Invoke-PackageSamCli {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "AWS SAM CLI (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $releaseInfo = Get-LatestSamCliRelease -Quiet
        if (-not $releaseInfo) { exit 1 }
        Write-Output $releaseInfo.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("AWS SAM CLI GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "AWS SAM CLI (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-awssamcli'
