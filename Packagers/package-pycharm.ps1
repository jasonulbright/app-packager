...
.SYNOPSIS
    Packages PyCharm Community (x64) for MECM.
...
.DESCRIPTION
    ...
    presence of the versioned ARP uninstall key ("PyCharm <version>").
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available PyCharm version string and exits.
...
function Invoke-StagePyCharm {
    ...
    # --- Detection key ---
    $arpKeyRelative = "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\PyCharm $version"
...
if ($GetLatestVersionOnly) {
    try {
        ...
        [Console]::Error.WriteLine("PyCharm GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-pycharm'
