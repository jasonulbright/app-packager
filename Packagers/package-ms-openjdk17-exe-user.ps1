<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 17 (x64, EXE, per user)
CMName: Microsoft OpenJDK 17 (EXE, per user)
...
.SYNOPSIS
    Packages Microsoft Build of OpenJDK 17 (x64) EXE, per user, for ConfigMgr.
...
.DESCRIPTION
    ...
    local folder, and creates a user-context ConfigMgr Application with HKCU
    ARP DisplayVersion detection on the Inno Setup ARP entry.

    The install runs /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP- /CURRENTUSER
...
$AppFolder        = "OpenJDK 17 (EXE, per user)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 17 (EXE, per user)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK17-EXE-User"

# Inno Setup derives the ARP key name from the compiled AppId, which stays the
# same across JDK 17 releases; setup runs in 64-bit mode.
$ArpRegistryKey = "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{65A75ED2-4165-6FAA-277E-7647F7E2F3EA}_is1"
...
function Invoke-StageMsOpenJdk17ExeUser {
    Write-Log ""
    Write-Log ("=" * 60)
    ...
    $wrappers = New-ExeWrapperContent -InstallerFileName $installerFileName `
        -InstallArgs "'/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART', '/SP-', '/CURRENTUSER', '/TASKS=`"FeatureEnvironment,FeatureJarFileRunWith`"'" `
    ...
    $uninstallContent = @'
$key = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{65A75ED2-4165-6FAA-277E-7647F7E2F3EA}_is1'
...
function Invoke-PackageMsOpenJdk17ExeUser {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk17ExeUser
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk17ExeUser
    }
    else {
        Invoke-StageMsOpenJdk17ExeUser
        Invoke-PackageMsOpenJdk17ExeUser
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk17-exe-user'
