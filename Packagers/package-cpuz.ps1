...
.SYNOPSIS
    Packages CPU-Z (x64) for MECM.

.DESCRIPTION
    Reads the current version from the vendor product page, downloads the
    matching English Inno Setup installer, stages content to a versioned local
    folder, and creates an MECM Application with file-existence detection on
    the installed cpuz_x64.exe.
...
.PARAMETER DownloadRoot
    ...
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\CPU-Z).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available CPU-Z version string and exits.
...
# --- Configuration ---
$DownloadPageUrl = "https://www.cpuid.com/softwares/cpu-z.html"
$DownloadBase    = "https://www.cpuid.com/downloads/cpu-z"
...
$InstallDir = "{0}\CPUID\CPU-Z" -f $env:ProgramFiles

# --- Functions ---


function Assert-CpuZPayloadIsExecutable {
    <#
    .SYNOPSIS
...
function Resolve-CpuZInstallerUrl {
    <#
    .SYNOPSIS
        Returns the direct installer URL for a CPU-Z version.
    .DESCRIPTION
    ...
    $fallback = "https://download.cpuid.com/cpu-z/$FileName"
...
function Get-LatestCpuZVersion {
    <#
    .SYNOPSIS
        Returns the highest CPU-Z version linked on the vendor product page.
    .DESCRIPTION
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "Failed to fetch CPU-Z download page: $DownloadPageUrl" }

        $rx = [regex]'cpu-z_(?<ver>\d+\.\d+)-en\.exe'
        $rxMatches = $rx.Matches($html)
        if (-not $rxMatches -or $rxMatches.Count -lt 1) {
            throw "Could not locate any English installer links on the download page."
        }
        ...
        Write-Log "Latest CPU-Z version         : $best" -Quiet:$Quiet
        return $best
    }
    catch {
...
function Invoke-StageCpuZ {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "CPU-Z (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Get version ---
    $version = Get-LatestCpuZVersion
    if (-not $version) { throw "Could not resolve CPU-Z version." }

    $installerFileName = "cpu-z_$version-en.exe"
    $downloadUrl       = Resolve-CpuZInstallerUrl -FileName $installerFileName
    ...
    Assert-CpuZPayloadIsExecutable -Path $localExe
...
foreach ($root in $keys) {
    ...
    $hit = Get-ChildItem -LiteralPath $root -ErrorAction SilentlyContinue |
        ...
        Where-Object { $_.DisplayName -like 'CPUID CPU-Z*' -or $_.DisplayName -like 'CPU-Z*' } |
...
function Invoke-PackageCpuZ {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "CPU-Z (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $v = Get-LatestCpuZVersion -Quiet
        if (-not $v) { exit 1 }
        Write-Output $v
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("CPU-Z GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "CPU-Z (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-cpuz'
