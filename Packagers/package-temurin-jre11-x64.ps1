...
.SYNOPSIS
    Packages Eclipse Temurin JRE 11 (x64) MSI for ConfigMgr.
...
function Invoke-StageTemurinJRE11X64 {
    ...
    # Normalize AppName: Temurin MSI ProductName is verbose
    # ("Eclipse Temurin JRE with Hotspot 11.0.30+7 (x64)"). Construct a
    # short, stable AppName from $FeatureVersion, $ImageType, $Architecture.
    $appName = "Eclipse Temurin $($ImageType.ToUpper()) $FeatureVersion ($Architecture)"
...
try {
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-temurin-jre11-x64'
