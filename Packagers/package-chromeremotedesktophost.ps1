<#
Vendor: Google
App: Chrome Remote Desktop Host
CMName: Chrome Remote Desktop Host
VendorUrl: https://remotedesktop.google.com/
...
.SYNOPSIS
    Packages the Chrome Remote Desktop Host (x64) MSI for MECM.

.DESCRIPTION
    Downloads the Chrome Remote Desktop Host MSI from Google's evergreen
    download URL, stages content to a versioned local folder with ARP detection
    ...
    NOTE: The download URL always serves the current release; Google publishes no
    version endpoint for the host, so the version is read from MSI properties
    after download.
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Google\Chrome Remote Desktop Host\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\ChromeRemoteDesktopHost).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Downloads the MSI, outputs its ProductVersion, and exits.
...
# --- Configuration ---
$MsiDownloadUrl = "https://dl.google.com/edgedl/chrome-remote-desktop/chromeremotedesktophost.msi"
$MsiFileName    = "chromeremotedesktophost.msi"

$VendorFolder = "Google"
$AppFolder    = "Chrome Remote Desktop Host"

$BaseDownloadRoot = Join-Path $DownloadRoot "ChromeRemoteDesktopHost"
...
function Invoke-StageCrdHost {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Chrome Remote Desktop Host (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "Google LLC" }

    $appName = $productName
    if ([string]::IsNullOrWhiteSpace($appName)) { $appName = "Chrome Remote Desktop Host" }
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        ...
        RunningProcess  = @("remoting_host", "remoting_desktop")
        Detection       = @{
...
function Invoke-PackageCrdHost {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Chrome Remote Desktop Host (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $localMsi = Join-Path $BaseDownloadRoot $MsiFileName
        Invoke-DownloadWithRetry -Url $MsiDownloadUrl -OutFile $localMsi -Quiet
        Assert-MsiPayload -Path $localMsi
        ...
        [Console]::Error.WriteLine("Chrome Remote Desktop Host GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Chrome Remote Desktop Host (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-chromeremotedesktophost'
