<#
Vendor: Yubico
App: YubiKey Manager CLI
CMName: YubiKey Manager CLI
VendorUrl: https://developers.yubico.com/yubikey-manager/
...
.DESCRIPTION
    Downloads the latest win64 MSI from the official GitHub releases API of the
    yubikey-manager project, stages content to a versioned local folder with ARP
    detection derived from the MSI properties, and creates a ConfigMgr Application
    with registry-based detection.

    This is the command-line tool (ykman). The graphical YubiKey Manager was
    superseded by Yubico Authenticator and is packaged separately.
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Yubico\YubiKey Manager CLI\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\YubiKeyManagerCLI).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available YubiKey Manager CLI version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/Yubico/yubikey-manager/releases/latest"

$VendorFolder = "Yubico"
$AppFolder    = "YubiKey Manager CLI"

$BaseDownloadRoot = Join-Path $DownloadRoot "YubiKeyManagerCLI"
...
function Assert-MsiPayload {
    ...
    .DESCRIPTION
        ...
        HTML, which would otherwise stage as a valid-looking MSI.
    #>
...
function Get-LatestYubiKeyManagerCliRelease {
    param([switch]$Quiet)
    ...
    try {
        ...
        # Anchored so the detached .sig signature sharing the asset name is not taken.
        $rx = '^yubikey-manager-(?<ver>\d+(?:\.\d+)+)-win64\.msi$'
        $asset = $release.assets | Where-Object { $_.name -match $rx } | Select-Object -First 1
        if (-not $asset) {
            throw "Could not find a win64 MSI in the latest GitHub release."
        }
        ...
        Write-Log "Failed to get YubiKey Manager CLI version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageYubiKeyManagerCli {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "YubiKey Manager CLI (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Get version ---
    $releaseInfo = Get-LatestYubiKeyManagerCliRelease
    if (-not $releaseInfo) { throw "Could not resolve YubiKey Manager CLI version." }
...
function Invoke-PackageYubiKeyManagerCli {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "YubiKey Manager CLI (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestYubiKeyManagerCliRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("YubiKey Manager CLI GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "YubiKey Manager CLI (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageYubiKeyManagerCli
    }
    elseif ($PackageOnly) {
        Invoke-PackageYubiKeyManagerCli
    }
    else {
        Invoke-StageYubiKeyManagerCli
        Invoke-PackageYubiKeyManagerCli
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-yubikeymanagercli'
