<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 21 (x64, EXE, per user)
CMName: Microsoft OpenJDK 21 (EXE, per user)
...
.SYNOPSIS
    Packages Microsoft Build of OpenJDK 21 (x64) EXE, per user, for ConfigMgr.
...
$AppFolder        = "OpenJDK 21 (EXE, per user)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 21 (EXE, per user)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK21-EXE-User"

# Inno Setup derives the ARP key name from the compiled AppId, which stays the
# same across JDK 21 releases; setup runs in 64-bit mode.
$ArpRegistryKey = "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{11F8EC6C-68A4-9099-96F9-E81566D319CC}_is1"
...
function Invoke-StageMsOpenJdk21ExeUser {
    Write-Log ""
    Write-Log ("=" * 60)
    ...
    $uninstallContent = @'
$key = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{11F8EC6C-68A4-9099-96F9-E81566D319CC}_is1'
...
function Invoke-PackageMsOpenJdk21ExeUser {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk21ExeUser
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk21ExeUser
    }
    else {
        Invoke-StageMsOpenJdk21ExeUser
        Invoke-PackageMsOpenJdk21ExeUser
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk21-exe-user'
