...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available NAPS2 version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/cyanfish/naps2/releases/latest"

$VendorFolder = "NAPS2"
$AppFolder    = "NAPS2"

$BaseDownloadRoot = Join-Path $DownloadRoot "NAPS2"
...
function Get-LatestNaps2Release {
    <#
    .SYNOPSIS
        Returns the newest NAPS2 version and its win-x64 MSI URL.
    .DESCRIPTION
        Each release also carries win-x64 and win-arm64 exe installers and an
        arm64 MSI, so the asset list is filtered by exact name rather than the
        tag being trusted to name a downloadable file.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $m = [regex]::Match($json, '"browser_download_url":\s*"(?<url>[^"]*?/(?<file>naps2-(?<ver>\d+(?:\.\d+)+)-win-x64\.msi))"')
        if (-not $m.Success) { throw "Could not locate a win-x64 MSI asset in the latest release." }

        $version = $m.Groups['ver'].Value
        Write-Log "Latest NAPS2 version         : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get NAPS2 version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageNaps2 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NAPS2 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestNaps2Release
    if (-not $releaseInfo) { throw "Could not resolve NAPS2 version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "NAPS2" }
...
function Invoke-PackageNaps2 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NAPS2 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestNaps2Release -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("NAPS2 GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NAPS2 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-naps2'
