...
App: Amazon DCV Client
CMName: Amazon DCV Client
VendorUrl: https://www.amazondcv.com/
...
.SYNOPSIS
    Packages the Amazon DCV Client (x64) MSI for MECM.

.DESCRIPTION
    Downloads the current Amazon DCV Client MSI from the vendor's static
    ...
    The vendor publishes no version feed: the release URL always serves the
    current client and the version is read from the MSI after download.
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Amazon Web Services\Amazon DCV Client\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\AmazonDCV).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Downloads the current MSI, outputs its ProductVersion, and exits.
...
# --- Configuration ---
$MsiDownloadUrl = "https://d1uj6qtbmh3dt5.cloudfront.net/nice-dcv-client-Release.msi"
$MsiFileName    = "nice-dcv-client-Release.msi"

$VendorFolder = "Amazon Web Services"
$AppFolder    = "Amazon DCV Client"

$BaseDownloadRoot = Join-Path $DownloadRoot "AmazonDCV"
...
function Get-AmazonDcvMsi {
    <#
    .SYNOPSIS
        Downloads the current client MSI into the download root and returns its path.
    #>
    param([switch]$Quiet)
...
function Invoke-StageAmazonDcv {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon DCV Client (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    # --- Download ---
    $localMsi = Get-AmazonDcvMsi
...
function Invoke-PackageAmazonDcv {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon DCV Client (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $localMsi = Get-AmazonDcvMsi -Quiet
        $props = Get-MsiPropertyMap -MsiPath $localMsi
        ...
        [Console]::Error.WriteLine("Amazon DCV Client GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Amazon DCV Client (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageAmazonDcv
    }
    elseif ($PackageOnly) {
        Invoke-PackageAmazonDcv
    }
    else {
        Invoke-StageAmazonDcv
        Invoke-PackageAmazonDcv
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-amazondcv'
