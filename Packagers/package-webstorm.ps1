...
App: WebStorm
CMName: WebStorm
VendorUrl: https://www.jetbrains.com/webstorm/
CPE: cpe:2.3:a:jetbrains:webstorm:*:*:*:*:*:*:*:*
...
.SYNOPSIS
    Packages WebStorm (x64) for MECM.

.DESCRIPTION
    Resolves the latest WebStorm release from the JetBrains product releases API,
    ...
    presence of the versioned ARP uninstall key ("WebStorm <version>").

    WebStorm requires a paid JetBrains subscription; the installer is
    unrestricted but the product prompts for a license on first run.
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available WebStorm version string and exits.
...
# --- Configuration ---
$ReleasesApiUrl = "https://data.services.jetbrains.com/products/releases?code=WS&latest=true&type=release"

$VendorFolder = "JetBrains"
$AppFolder    = "WebStorm"

$BaseDownloadRoot = Join-Path $DownloadRoot "WebStorm"
...
function Get-LatestWebStormRelease {
    <#
    .SYNOPSIS
        Returns the newest WebStorm release version and its Windows installer URL.
    .DESCRIPTION
        ...
        The windowsARM64 entry is deliberately ignored: this packager ships x64.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $release = $data.WS | Select-Object -First 1
        if (-not $release) { throw "The releases API returned no WS entries." }
        ...
        Write-Log "Latest WebStorm version      : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get WebStorm version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageWebStorm {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "WebStorm (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestWebStormRelease
    if (-not $releaseInfo) { throw "Could not resolve WebStorm version." }
    ...
    if (-not (Test-Path -LiteralPath $localExe)) {
        Write-Log "Downloading WebStorm..."
    ...
    # --- Detection key ---
    $arpKeyRelative = "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\WebStorm $version"
...
function Invoke-PackageWebStorm {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "WebStorm (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestWebStormRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("WebStorm GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "WebStorm (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageWebStorm
    }
    elseif ($PackageOnly) {
        Invoke-PackageWebStorm
    }
    else {
        Invoke-StageWebStorm
        Invoke-PackageWebStorm
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-webstorm'
