...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Zeal\Zeal\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\Zeal).
    Default: C:\temp\ap
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/zealdocs/zeal/releases/latest"

$VendorFolder = "Zeal"
$AppFolder    = "Zeal"

$BaseDownloadRoot = Join-Path $DownloadRoot "Zeal"
...
function Get-LatestZealRelease {
    <#
    .SYNOPSIS
        Returns the newest Zeal version and its windows-x64 MSI URL.
    .DESCRIPTION
        The release also carries an arm64 MSI and portable zip/7z archives; the
        anchored pattern keeps the per-machine x64 MSI.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $rx = '^zeal-(?<ver>\d+(?:\.\d+)+)-windows-x64\.msi$'
        $asset = $release.assets | Where-Object { $_.name -match $rx } | Select-Object -First 1
        if (-not $asset) {
            throw "Could not find a windows-x64 MSI in the latest GitHub release."
        }
        ...
        Write-Log "Failed to get Zeal version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageZeal {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Zeal (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Get version ---
    $releaseInfo = Get-LatestZealRelease
    if (-not $releaseInfo) { throw "Could not resolve Zeal version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "Zeal" }
...
function Invoke-PackageZeal {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Zeal (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestZealRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Zeal GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Zeal (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-zeal'
