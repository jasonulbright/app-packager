<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 17 (x64, MSI)
CMName: Microsoft OpenJDK 17 (MSI)
...
.SYNOPSIS
    Packages Microsoft Build of OpenJDK 17 (x64) MSI, per machine, for ConfigMgr.
...
$AppFolder        = "OpenJDK 17 (MSI)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 17 (MSI)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK17-MSI"
...
function Invoke-StageMsOpenJdk17Msi {
    Write-Log ""
    Write-Log ("=" * 60)
...
function Invoke-PackageMsOpenJdk17Msi {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk17Msi
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk17Msi
    }
    else {
        Invoke-StageMsOpenJdk17Msi
        Invoke-PackageMsOpenJdk17Msi
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk17-msi'
