...
App: ImageGlass (x64)
CMName: ImageGlass
VendorUrl: https://imageglass.org/
...
.SYNOPSIS
    Packages ImageGlass (x64) MSI for MECM.

.DESCRIPTION
    Resolves the newest stable release from the d2phap/ImageGlass GitHub
    repository, downloads the win-x64 MSI, stages content to a versioned local
...
$VendorFolder = "ImageGlass"
$AppFolder    = "ImageGlass"
$Publisher    = "Duong Dieu Phap"

$BaseDownloadRoot = Join-Path $DownloadRoot "ImageGlass"
...
function Get-LatestImageGlassRelease {
    <#
    .SYNOPSIS
        Returns the newest release version and its win-x64 MSI asset URL.
    .DESCRIPTION
        Each release also ships msix, zip, arm64, macOS and Linux assets, so the
        asset match is anchored on the win-x64 MSI name. Release tags carry the
        plain four-part version with no prefix.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $asset = $release.assets |
            Where-Object { $_.name -match '^ImageGlass_(?<ver>\d+(\.\d+)+)_win-x64\.msi$' } |
            Select-Object -First 1
        if (-not $asset) { throw "No win-x64 MSI asset found in release." }

        $version = ([regex]'^ImageGlass_(?<ver>\d+(\.\d+)+)_win-x64\.msi$').Match($asset.name).Groups['ver'].Value
        if ([string]::IsNullOrWhiteSpace($version)) {
            throw "Could not parse version from ImageGlass asset name."
        }
        ...
        Write-Log "Failed to get ImageGlass version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageImageGlass {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "ImageGlass (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestImageGlassRelease
    if (-not $releaseInfo) { throw "Could not resolve ImageGlass version." }
...
function Invoke-PackageImageGlass {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "ImageGlass (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestImageGlassRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("ImageGlass GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "ImageGlass (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageImageGlass
    }
    elseif ($PackageOnly) {
        Invoke-PackageImageGlass
    }
    else {
        Invoke-StageImageGlass
        Invoke-PackageImageGlass
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-imageglass'
