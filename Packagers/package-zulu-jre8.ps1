<#
Vendor: Azul
App: Azul Zulu JRE 8 (x64)
CMName: Azul Zulu JRE 8 (x64)
...
DownloadPageUrl: https://www.azul.com/downloads/?version=java-8-lts&os=windows&architecture=x86-64-bit&package=jre
IconSource: Installer
UpdateCadenceDays: 90

.SYNOPSIS
    Packages Azul Zulu JRE 8 (x64) MSI for ConfigMgr.
...
$VendorFolder   = "Azul"
$AppFolder      = "Zulu JRE 8 (x64)"
$Publisher      = "Azul Systems"
$BaseDownloadRoot = Join-Path $DownloadRoot "ZuluJRE8"

$DisplayTitle   = "Azul Zulu JRE 8 (x64)"
...
function Get-LatestZuluRelease {
    <#
    .SYNOPSIS
        Returns the newest GA Zulu 8 jre x64 Windows MSI for this feature version.
    .DESCRIPTION
    ...
    try {
        ...
        if ($candidates.Count -lt 1) {
            throw "No x64 Windows jre MSI package found for Zulu 8."
        }
        ...
        Write-Log "Failed to get Zulu 8 jre version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageZulu {
    ...
    if (-not $releaseInfo) { throw "Could not resolve Zulu 8 jre version." }
...
if ($GetLatestVersionOnly) {
    try {
        ...
        [Console]::Error.WriteLine("Zulu JRE 8 GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-zulu-jre8'
