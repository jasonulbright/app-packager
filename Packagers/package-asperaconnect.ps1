<#
Vendor: IBM
App: IBM Aspera Connect (x64)
CMName: IBM Aspera Connect
VendorUrl: https://www.ibm.com/aspera/connect/
...
.SYNOPSIS
    Packages IBM Aspera Connect (x64) MSI for MECM.

.DESCRIPTION
    Reads the current Windows x64 MSI link from the vendor download page,
    downloads it, stages content to a versioned local folder with ARP detection
...
$VendorFolder = "IBM"
$AppFolder    = "Aspera Connect"
$Publisher    = "IBM"

$BaseDownloadRoot = Join-Path $DownloadRoot "AsperaConnect"
...
function Assert-MsiPayload {
    ...
    .DESCRIPTION
        The download host answers 200 with an HTML error body for an expired
        link, which would otherwise stage as a valid-looking MSI and fail only
        at install time.
    #>
...
function Get-LatestAsperaConnectRelease {
    <#
    .SYNOPSIS
        Returns the current Windows x64 MSI version, filename and URL.
    .DESCRIPTION
        The download page links one MSI per platform and carries the version in
        the filename; the highest [version] wins rather than the first match in
        document order.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "Failed to fetch Aspera Connect download page: $DownloadPageUrl" }

        $rx = [regex]'href\s*=\s*"(?<url>https?://[^"]*?/(?<file>ibm-aspera-connect_(?<ver>\d+(\.\d+)+)[^"/]*_win32_AMD64\.msi))"'
        $rxMatches = $rx.Matches($html)
        if (-not $rxMatches -or $rxMatches.Count -lt 1) {
            throw "Could not locate a Windows x64 MSI link on the download page."
        }
        ...
        Write-Log "Failed to get Aspera Connect version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageAsperaConnect {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Aspera Connect (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestAsperaConnectRelease
    if (-not $releaseInfo) { throw "Could not resolve Aspera Connect version." }
    ...
    # The package is dual-purpose (ALLUSERS=2 with MSIINSTALLPERUSER=1), so an
    # unqualified install lands per-user. Clearing MSIINSTALLPERUSER alongside
    # ALLUSERS=1 selects the per-machine scope the directory tree already
    # targets (ProgramFiles64Folder\IBM\Aspera Connect).
    $wrapperContent = New-MsiWrapperContent -MsiFileName $msiFileName `
        -ExtraInstallArgs @("ALLUSERS=1", "MSIINSTALLPERUSER=")
    Write-ContentWrappers -OutputPath $localContentPath `
        -InstallPs1Content $wrapperContent.Install `
        -UninstallPs1Content $wrapperContent.Uninstall

    # --- Write stage manifest ---
    $appName = "IBM Aspera Connect (x64)"
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        ...
        InstallerType   = "MSI"
        InstallArgs     = "/qn /norestart ALLUSERS=1 MSIINSTALLPERUSER="
        UninstallArgs   = "/qn /norestart"
        ProductCode     = $productCode
        RunningProcess  = @("asperaconnect", "ascp")
        Detection       = @{
...
function Invoke-PackageAsperaConnect {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Aspera Connect (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestAsperaConnectRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Aspera Connect GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Aspera Connect (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageAsperaConnect
    }
    elseif ($PackageOnly) {
        Invoke-PackageAsperaConnect
    }
    else {
        Invoke-StageAsperaConnect
        Invoke-PackageAsperaConnect
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-asperaconnect'
