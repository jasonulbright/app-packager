...
App: Tabular Editor 2
CMName: Tabular Editor 2
VendorUrl: https://tabulareditor.com/
...
.DESCRIPTION
    ...
    a per-machine install. It is a 32-bit MSI, so the ARP entry lands in the
    WOW6432Node uninstall hive.
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Tabular Editor ApS\Tabular Editor 2\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\TabularEditor2).
    Default: C:\temp\ap
...
.REQUIREMENTS
    ...
    - Target machines need the .NET Framework 4.8 runtime
#>
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/TabularEditor/TabularEditor/releases/latest"

$VendorFolder = "Tabular Editor ApS"
$AppFolder    = "Tabular Editor 2"

$BaseDownloadRoot = Join-Path $DownloadRoot "TabularEditor2"

# The asset name is constant across releases, so the staged copy is renamed to
# carry the version and keep one cache entry per release.
$AssetName = "TabularEditor.Installer.msi"
...
function Get-LatestTabularEditor2Release {
    <#
    .SYNOPSIS
        Returns the latest Tabular Editor 2 version and its installer URL.
    .DESCRIPTION
        The repository also publishes a portable zip; only the MSI asset is
        usable here. The release tag carries the version because the asset name
        is version-free.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if ($version -notmatch '^\d+(\.\d+)+$') {
            throw "Release tag is not a plain version: $($release.tag_name)"
        }

        $asset = $release.assets | Where-Object { $_.name -eq $AssetName } | Select-Object -First 1
        if (-not $asset) {
            throw "Could not find '$AssetName' in the latest GitHub release."
        }

        Write-Log "Latest Tabular Editor 2      : $version" -Quiet:$Quiet

        return [pscustomobject]@{
            Version     = $version
            FileName    = "TabularEditor-$version.msi"
            DownloadUrl = $asset.browser_download_url
        }
    }
    catch {
        Write-Log "Failed to get Tabular Editor 2 version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageTabularEditor2 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Tabular Editor 2 - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Get version ---
    $releaseInfo = Get-LatestTabularEditor2Release
    if (-not $releaseInfo) { throw "Could not resolve Tabular Editor 2 version." }
    ...
    # ALLUSERS=1 puts the entry in the machine hive; the 32-bit package places
    # it under WOW6432Node, so the clause must not be evaluated as 64-bit.
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "Tabular Editor ApS" }

    $manifestPath = Join-Path $localContentPath "stage-manifest.json"
    Write-StageManifest -Path $manifestPath -ManifestData @{
        AppName         = "Tabular Editor 2"
        Publisher       = $publisher
...
function Invoke-PackageTabularEditor2 {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Tabular Editor 2 - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestTabularEditor2Release -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Tabular Editor 2 GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Tabular Editor 2 Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageTabularEditor2
    }
    elseif ($PackageOnly) {
        Invoke-PackageTabularEditor2
    }
    else {
        Invoke-StageTabularEditor2
        Invoke-PackageTabularEditor2
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-tabulareditor2'
