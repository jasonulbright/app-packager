...
CMName: DB Browser for SQLite
VendorUrl: https://sqlitebrowser.org/
...
.SYNOPSIS
    Packages DB Browser for SQLite (x64) MSI for MECM.

.DESCRIPTION
    Resolves the latest release from the sqlitebrowser GitHub releases API,
    downloads the win64 MSI, stages content to a versioned local folder with
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/sqlitebrowser/sqlitebrowser/releases/latest"

$VendorFolder = "DB Browser for SQLite"
$AppFolder    = "DB Browser for SQLite"

$BaseDownloadRoot = Join-Path $DownloadRoot "DB Browser for SQLite"
...
function Get-LatestSqliteBrowserRelease {
    <#
    .SYNOPSIS
        Returns the newest release version and its win64 MSI asset URL.
    .DESCRIPTION
        Release tags carry a v prefix. Each release also ships win32, arm64 and
        portable assets, so the asset match is anchored on the win64 MSI name.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $asset = $release.assets | Where-Object { $_.name -match '^DB\.Browser\.for\.SQLite-v.*-win64\.msi$' } | Select-Object -First 1
        if (-not $asset) { throw "No win64 MSI asset found in release." }
        ...
        Write-Log "Failed to get DB Browser for SQLite version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageSqliteBrowser {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "DB Browser for SQLite (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestSqliteBrowserRelease
    if (-not $releaseInfo) { throw "Could not resolve DB Browser for SQLite version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "DB Browser for SQLite Team" }
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        ...
        RunningProcess  = @("DB Browser for SQLite")
        Detection       = @{
...
function Invoke-PackageSqliteBrowser {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "DB Browser for SQLite (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestSqliteBrowserRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("DB Browser for SQLite GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "DB Browser for SQLite (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageSqliteBrowser
    }
    elseif ($PackageOnly) {
        Invoke-PackageSqliteBrowser
    }
    else {
        Invoke-StageSqliteBrowser
        Invoke-PackageSqliteBrowser
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-dbbrowsersqlite'
