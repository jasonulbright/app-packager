<#
Vendor: IBM
App: IBM Semeru Runtime Open Edition JRE 11 (x64)
CMName: IBM Semeru Runtime Open Edition JRE 11
...
$VendorFolder   = "IBM"
$AppFolder      = "Semeru JRE 11"
$Publisher      = "IBM"
$BaseDownloadRoot = Join-Path $DownloadRoot "SemeruJRE11"
...
function Get-LatestSemeruRelease {
    ...
    try {
        ...
        if ($candidates.Count -lt 1) {
            throw "No x64 Windows jre MSI asset found in any stable Semeru 11 release."
        }
...
function Invoke-StageSemeru {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Semeru Runtime Open Edition JRE 11 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
function Invoke-PackageSemeru {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Semeru Runtime Open Edition JRE 11 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        [Console]::Error.WriteLine("Semeru JRE 11 GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Semeru Runtime Open Edition JRE 11 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-semeru-jre11'
