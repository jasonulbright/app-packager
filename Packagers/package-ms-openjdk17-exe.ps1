<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 17 (x64, EXE)
CMName: Microsoft OpenJDK 17 (EXE)
...
.SYNOPSIS
    Packages Microsoft Build of OpenJDK 17 (x64) EXE, per machine, for ConfigMgr.
...
$AppFolder        = "OpenJDK 17 (EXE)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 17 (EXE)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK17-EXE"
...
function Invoke-StageMsOpenJdk17Exe {
    Write-Log ""
    Write-Log ("=" * 60)
    ...
    $uninstallContent = @'
$key = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{65A75ED2-4165-6FAA-277E-7647F7E2F3EA}_is1'
...
function Invoke-PackageMsOpenJdk17Exe {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk17Exe
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk17Exe
    }
    else {
        Invoke-StageMsOpenJdk17Exe
        Invoke-PackageMsOpenJdk17Exe
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk17-exe'
