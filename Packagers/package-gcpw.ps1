...
CMName: Google Credential Provider for Windows
VendorUrl: https://tools.google.com/dlpage/gcpw/
...
.SYNOPSIS
    Packages Google Credential Provider for Windows (x64) MSI for MECM.

.DESCRIPTION
    Downloads the GCPW standalone enterprise MSI from Google's evergreen
    ...
    NOTE: The MSI installs the credential provider only. Sign-in is refused until
    HKLM\SOFTWARE\Google\GCPW carries the tenant policy values (at minimum
    domains_allowed_to_login); that policy is delivered separately.
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Google\Google Credential Provider for Windows\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\GCPW).
    Default: C:\temp\ap
...
# --- Configuration ---
$MsiDownloadUrl = "https://dl.google.com/dl/credentialprovider/gcpwstandaloneenterprise64.msi"
$MsiFileName    = "gcpwstandaloneenterprise64.msi"

$VendorFolder = "Google"
$AppFolder    = "Google Credential Provider for Windows"

$BaseDownloadRoot = Join-Path $DownloadRoot "GCPW"
...
function Invoke-StageGcpw {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Google Credential Provider for Windows (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    $appName = $productName
    if ([string]::IsNullOrWhiteSpace($appName)) { $appName = "Google Credential Provider for Windows" }
...
function Invoke-PackageGcpw {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Google Credential Provider for Windows (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        [Console]::Error.WriteLine("GCPW GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Google Credential Provider for Windows (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-gcpw'
