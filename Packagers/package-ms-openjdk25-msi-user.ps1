<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 25 (x64, MSI, per user)
CMName: Microsoft OpenJDK 25 (MSI, per user)
...
.SYNOPSIS
    Packages Microsoft Build of OpenJDK 25 (x64) MSI, per user, for ConfigMgr.
...
$AppFolder        = "OpenJDK 25 (MSI, per user)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 25 (MSI, per user)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK25-MSI-User"
...
function Invoke-StageMsOpenJdk25MsiUser {
    Write-Log ""
    Write-Log ("=" * 60)
...
function Invoke-PackageMsOpenJdk25MsiUser {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk25MsiUser
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk25MsiUser
    }
    else {
        Invoke-StageMsOpenJdk25MsiUser
        Invoke-PackageMsOpenJdk25MsiUser
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk25-msi-user'
