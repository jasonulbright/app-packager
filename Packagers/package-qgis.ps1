...
.DESCRIPTION
    ...
    latest release; package-qgis-ltr.ps1 follows the long term release.
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available QGIS version string and exits.
...
$BaseDownloadRoot = Join-Path $DownloadRoot "QGIS"
...
function Get-LatestQgisRelease {
    <#
    .SYNOPSIS
        Returns the current QGIS release version and its MSI URL.
    .DESCRIPTION
        The download page offers exactly two Windows MSIs, the long term release
        and the current release; the current release is the higher version of
    ...
    try {
        ...
        Write-Log "Latest QGIS version          : $($best.Version)" -Quiet:$Quiet
        ...
        Write-Log "Failed to get QGIS version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageQgis {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "QGIS (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestQgisRelease
    if (-not $releaseInfo) { throw "Could not resolve QGIS version." }
...
function Invoke-PackageQgis {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "QGIS (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestQgisRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("QGIS GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "QGIS (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-qgis'
