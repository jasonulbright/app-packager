<#
Vendor: IBM
App: IBM Semeru Runtime Open Edition JDK 17 (x64)
CMName: IBM Semeru Runtime Open Edition JDK 17
VendorUrl: https://developer.ibm.com/languages/java/semeru-runtimes/
CPE: cpe:2.3:a:ibm:semeru_runtime:*:*:*:*:open:*:*:*
ReleaseNotesUrl: https://github.com/ibmruntimes/semeru17-binaries/releases
DownloadPageUrl: https://developer.ibm.com/languages/java/semeru-runtimes/downloads/
IconSource: Installer
UpdateCadenceDays: 90

.SYNOPSIS
    Packages IBM Semeru Runtime Open Edition JDK 17 (x64) MSI for ConfigMgr.

.DESCRIPTION
    Resolves the newest stable release from the ibmruntimes/semeru17-binaries
    GitHub repository, downloads the x64 Windows jdk MSI, stages content to a
...
$ImageType      = "jdk"
$Architecture   = "x64"
$GitHubApiUrl   = "https://api.github.com/repos/ibmruntimes/semeru17-binaries/releases?per_page=100"
$VendorFolder   = "IBM"
$AppFolder      = "Semeru JDK 17"
$Publisher      = "IBM"
$BaseDownloadRoot = Join-Path $DownloadRoot "SemeruJDK17"
...
function Get-LatestSemeruRelease {
    <#
    .SYNOPSIS
        Returns the newest stable Semeru 17 release version and its x64 Windows
        jdk MSI asset URL.
    .DESCRIPTION
        The repository publishes backport and early-access releases after newer
        general-availability ones, so publication order does not track version
        order and every candidate is sorted as a version instead. Releases that
        carry no x64 Windows jdk MSI are skipped.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if (-not $releases) { throw "GitHub releases API returned no releases." }

        $assetRx = [regex]"^ibm-semeru-open-$ImageType`_x64_windows_(?<ver>\d+(\.\d+)+)\.msi$"

        $candidates = @()
        foreach ($rel in $releases) {
            if ($rel.draft -or $rel.prerelease) { continue }
            foreach ($asset in $rel.assets) {
                $m = $assetRx.Match($asset.name)
        ...
        if ($candidates.Count -lt 1) {
            throw "No x64 Windows jdk MSI asset found in any stable Semeru 17 release."
        }
        ...
        Write-Log "Failed to get Semeru 17 version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageSemeru {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Semeru Runtime Open Edition JDK 17 (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestSemeruRelease
    if (-not $releaseInfo) { throw "Could not resolve Semeru 17 version." }
    ...
    $appName = "IBM Semeru Runtime Open Edition JDK $FeatureVersion ($Architecture)"
...
function Invoke-PackageSemeru {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Semeru Runtime Open Edition JDK 17 (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestSemeruRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Semeru JDK 17 GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IBM Semeru Runtime Open Edition JDK 17 (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-semeru-jdk17'
