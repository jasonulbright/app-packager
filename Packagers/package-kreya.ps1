...
.SYNOPSIS
    Packages Kreya (x64) MSI for MECM.

.DESCRIPTION
    Resolves the current release from the vendor download page, downloads the
    x64 MSI from the vendor's stable download host, stages content to a
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\riok GmbH\Kreya\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\Kreya).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available Kreya version string and exits.
...
$VendorFolder = "riok GmbH"
$AppFolder    = "Kreya"

$BaseDownloadRoot = Join-Path $DownloadRoot "Kreya"
$MsiFileName      = "Kreya-win-x64.msi"
...
function Get-LatestKreyaRelease {
    <#
    .SYNOPSIS
        Returns the newest Kreya version and its x64 MSI URL.
    .DESCRIPTION
        The download host has no listable index; the version lives only in the
        download page's asset links, which are versioned per release. The page
        also links the CLI archives for three platforms, so the match is pinned
        to the Windows x64 MSI, and the highest linked version wins rather than
        document order.
    #>
    param([switch]$Quiet)

    Write-Log "Kreya download page          : $DownloadPageUrl" -Quiet:$Quiet

    try {
        $html = (curl.exe -L --fail --silent --show-error $DownloadPageUrl) -join "`n"
        if ($LASTEXITCODE -ne 0) { throw "Failed to fetch the Kreya download page: $DownloadPageUrl" }

        $rx = [regex]('(?i)(?<url>https://[^"'' ]*/(?<ver>\d+(?:\.\d+)+)/' + [regex]::Escape($MsiFileName) + ')')
        ...
        Write-Log "Latest Kreya version         : $($best.Version)" -Quiet:$Quiet
        ...
        Write-Log "Failed to get Kreya version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageKreya {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Kreya (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestKreyaRelease
    if (-not $releaseInfo) { throw "Could not resolve Kreya version." }
    ...
    # --- Download ---
    # The asset name is version-free, so the cached copy is kept per version.
    $localMsi = Join-Path $BaseDownloadRoot ("Kreya-win-x64-{0}.msi" -f $version)
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "riok GmbH" }
...
function Invoke-PackageKreya {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Kreya (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestKreyaRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Kreya GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Kreya (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-kreya'
