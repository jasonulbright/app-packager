...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available NetBird version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/netbirdio/netbird/releases/latest"

$VendorFolder = "NetBird"
$AppFolder    = "NetBird"

$BaseDownloadRoot = Join-Path $DownloadRoot "NetBird"
...
function Get-LatestNetBirdRelease {
    <#
    .SYNOPSIS
        Returns the newest NetBird version and its windows_amd64 MSI URL.
    .DESCRIPTION
        A release publishes CLI tarballs and arm64 payloads alongside the
        Windows installers, and ships both an exe and an MSI for each
        architecture, so the asset list is filtered by exact name.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $m = [regex]::Match($json, '"browser_download_url":\s*"(?<url>[^"]*?/(?<file>netbird_installer_(?<ver>\d+(?:\.\d+)+)_windows_amd64\.msi))"')
        if (-not $m.Success) { throw "Could not locate a windows_amd64 MSI asset in the latest release." }

        $version = $m.Groups['ver'].Value
        Write-Log "Latest NetBird version       : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get NetBird version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageNetBird {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NetBird (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestNetBirdRelease
    if (-not $releaseInfo) { throw "Could not resolve NetBird version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "NetBird" }
...
function Invoke-PackageNetBird {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NetBird (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestNetBirdRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("NetBird GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NetBird (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-netbird'
