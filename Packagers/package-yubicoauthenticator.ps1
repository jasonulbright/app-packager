...
CMName: Yubico Authenticator
VendorUrl: https://www.yubico.com/products/yubico-authenticator/
...
.DESCRIPTION
    ...
    yubioath-flutter project, stages content to a versioned local folder with ARP
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\Yubico\Yubico Authenticator\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\YubicoAuthenticator).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available Yubico Authenticator version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/Yubico/yubioath-flutter/releases/latest"

$VendorFolder = "Yubico"
$AppFolder    = "Yubico Authenticator"

$BaseDownloadRoot = Join-Path $DownloadRoot "YubicoAuthenticator"
...
function Get-LatestYubicoAuthenticatorRelease {
    param([switch]$Quiet)
    ...
    try {
        ...
        $rx = '^yubico-authenticator-(?<ver>\d+(?:\.\d+)+)-win64\.msi$'
        ...
        Write-Log "Failed to get Yubico Authenticator version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageYubicoAuthenticator {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Yubico Authenticator (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Get version ---
    $releaseInfo = Get-LatestYubicoAuthenticatorRelease
    if (-not $releaseInfo) { throw "Could not resolve Yubico Authenticator version." }
...
function Invoke-PackageYubicoAuthenticator {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Yubico Authenticator (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestYubicoAuthenticatorRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Yubico Authenticator GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Yubico Authenticator (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageYubicoAuthenticator
    }
    elseif ($PackageOnly) {
        Invoke-PackageYubicoAuthenticator
    }
    else {
        Invoke-StageYubicoAuthenticator
        Invoke-PackageYubicoAuthenticator
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-yubicoauthenticator'
