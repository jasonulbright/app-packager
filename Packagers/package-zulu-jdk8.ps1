<#
Vendor: Azul
App: Azul Zulu JDK 8 (x64)
CMName: Azul Zulu JDK 8 (x64)
VendorUrl: https://www.azul.com/downloads/?package=jdk
CPE: cpe:2.3:a:azul:zulu:*:*:*:*:*:*:*:*
ReleaseNotesUrl: https://docs.azul.com/core/release-notes
DownloadPageUrl: https://www.azul.com/downloads/?version=java-8-lts&os=windows&architecture=x86-64-bit&package=jdk
IconSource: Installer
UpdateCadenceDays: 90

.SYNOPSIS
    Packages Azul Zulu JDK 8 (x64) MSI for ConfigMgr.
...
$VendorFolder   = "Azul"
$AppFolder      = "Zulu JDK 8 (x64)"
$Publisher      = "Azul Systems"
$BaseDownloadRoot = Join-Path $DownloadRoot "ZuluJDK8"

$DisplayTitle   = "Azul Zulu JDK 8 (x64)"
...
function Get-LatestZuluRelease {
    <#
    .SYNOPSIS
        Returns the newest GA Zulu 8 jdk x64 Windows MSI for this feature version.
    .DESCRIPTION
    ...
    try {
        ...
        if ($candidates.Count -lt 1) {
            throw "No x64 Windows jdk MSI package found for Zulu 8."
        }
        ...
        Write-Log "Failed to get Zulu 8 jdk version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageZulu {
    ...
    if (-not $releaseInfo) { throw "Could not resolve Zulu 8 jdk version." }
...
if ($GetLatestVersionOnly) {
    try {
        ...
        [Console]::Error.WriteLine("Zulu JDK 8 GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-zulu-jdk8'
