...
.SYNOPSIS
    Packages Pandoc (x64) MSI for MECM.

.DESCRIPTION
    Resolves the latest stable release from the jgm/pandoc GitHub releases API,
    downloads the windows-x86_64 MSI, stages content to a versioned local folder
...
.PARAMETER FileServerPath
    ...
    Content is staged under: <FileServerPath>\Applications\John MacFarlane\Pandoc\<Version>

.PARAMETER DownloadRoot
    Local root folder for staging downloaded installers.
    Each packager creates a subfolder under this path (e.g., <DownloadRoot>\Pandoc).
    Default: C:\temp\ap
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available Pandoc version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/jgm/pandoc/releases/latest"

$VendorFolder = "John MacFarlane"
$AppFolder    = "Pandoc"

$BaseDownloadRoot = Join-Path $DownloadRoot "Pandoc"

# The package sets MSIINSTALLPERUSER=1 and leaves ALLUSERS unset, which makes
# per-user the default context. ALLUSERS=1 forces the per-machine context and
# makes the installer ignore MSIINSTALLPERUSER.
...
function Get-LatestPandocRelease {
    <#
    .SYNOPSIS
        Returns the latest Pandoc version and its windows-x86_64 MSI asset URL.
    .DESCRIPTION
        The releases/latest endpoint excludes prereleases, so the newest stable
        tag is the one returned. The version comes from the asset name rather
        than the tag so it always matches the staged file.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        $rx = '^pandoc-(?<ver>\d+(?:\.\d+)+)-windows-x86_64\.msi$'
        ...
        Write-Log "Latest Pandoc version        : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get Pandoc version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StagePandoc {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Pandoc (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Get version ---
    $releaseInfo = Get-LatestPandocRelease
    if (-not $releaseInfo) { throw "Could not resolve Pandoc version." }
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "John MacFarlane" }
...
function Invoke-PackagePandoc {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Pandoc (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestPandocRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Pandoc GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Pandoc (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-pandoc'
