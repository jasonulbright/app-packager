<#
Vendor: Mattermost
App: Mattermost Desktop (x64)
CMName: Mattermost Desktop
...
# --- Configuration ---
$ReleasesApiUrl = "https://api.github.com/repos/mattermost/desktop/releases/latest"

$VendorFolder = "Mattermost"
$AppFolder    = "Mattermost Desktop"
$Publisher    = "Mattermost, Inc."

$BaseDownloadRoot = Join-Path $DownloadRoot "MattermostDesktop"
...
function Assert-MsiPayload {
    ...
    .DESCRIPTION
        A release asset withdrawn between the API query and the download answers
        with an HTML body, which would otherwise stage as a valid-looking MSI and
...
function Get-LatestMattermostRelease {
    <#
    .SYNOPSIS
        Returns the current Mattermost Desktop version and its x64 MSI URL.
    .DESCRIPTION
        /releases/latest already excludes prereleases and drafts. The release
        also carries an arm64 MSI, so the asset is matched on the win-x64
        suffix rather than on being the only MSI present.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($tag)) { throw "Release has no tag_name." }

        $version = $tag -replace '^v', ''
        if ($version -notmatch '^\d+(\.\d+)+$') { throw "Unexpected Mattermost Desktop tag: $tag" }

        $asset = @($release.assets | Where-Object { $_.name -like '*win-x64.msi' }) | Select-Object -First 1
        if (-not $asset) { throw "Release $tag carries no win-x64 MSI asset." }

        Write-Log "Latest Mattermost Desktop    : $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get Mattermost Desktop version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageMattermost {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Mattermost Desktop (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    # --- Get version ---
    $release = Get-LatestMattermostRelease
    if (-not $release) { throw "Could not resolve Mattermost Desktop version." }
    ...
    Write-StageManifest -Path $manifestPath -ManifestData @{
        AppName         = "Mattermost Desktop (x64)"
        Publisher       = $publisher
...
function Invoke-PackageMattermost {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Mattermost Desktop (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestMattermostRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Mattermost Desktop GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Mattermost Desktop (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageMattermost
    }
    elseif ($PackageOnly) {
        Invoke-PackageMattermost
    }
    else {
        Invoke-StageMattermost
        Invoke-PackageMattermost
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-mattermost'
