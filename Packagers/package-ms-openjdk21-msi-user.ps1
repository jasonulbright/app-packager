<#
Vendor: Microsoft
App: Microsoft Build of OpenJDK 21 (x64, MSI, per user)
CMName: Microsoft OpenJDK 21 (MSI, per user)
...
.SYNOPSIS
    Packages Microsoft Build of OpenJDK 21 (x64) MSI, per user, for ConfigMgr.
...
$AppFolder        = "OpenJDK 21 (MSI, per user)"
$Publisher        = "Microsoft"
$AppName          = "Microsoft OpenJDK 21 (MSI, per user)"
$BaseDownloadRoot = Join-Path $DownloadRoot "MsOpenJDK21-MSI-User"
...
function Invoke-StageMsOpenJdk21MsiUser {
    Write-Log ""
    Write-Log ("=" * 60)
...
function Invoke-PackageMsOpenJdk21MsiUser {
    Write-Log ""
    Write-Log ("=" * 60)
...
try {
    ...
    if ($StageOnly) {
        Invoke-StageMsOpenJdk21MsiUser
    }
    elseif ($PackageOnly) {
        Invoke-PackageMsOpenJdk21MsiUser
    }
    else {
        Invoke-StageMsOpenJdk21MsiUser
        Invoke-PackageMsOpenJdk21MsiUser
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-ms-openjdk21-msi-user'
