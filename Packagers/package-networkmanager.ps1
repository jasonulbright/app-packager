...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available NETworkManager version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/BornToBeRoot/NETworkManager/releases/latest"

$VendorFolder = "BornToBeRoot"
$AppFolder    = "NETworkManager"

$BaseDownloadRoot = Join-Path $DownloadRoot "NETworkManager"
...
function Get-LatestNetworkManagerRelease {
    <#
    .SYNOPSIS
        Returns the newest stable NETworkManager release and its Setup MSI asset.
    .DESCRIPTION
        The repository publishes preview builds as prereleases, so a prerelease
        flag or a non-numeric tag is treated as a resolution failure rather than
        staged as stable. The release also carries an Archive and a Portable zip,
        so the asset match requires the Setup MSI.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if ($release.prerelease) { throw "Latest NETworkManager release is flagged prerelease; refusing to stage." }

        $version = $release.tag_name -replace '^v', ''
        if ($version -notmatch '^\d+(\.\d+){2,3}$') {
            throw "GitHub release tag is not a stable numeric version: $($release.tag_name)"
        }

        $asset = $release.assets |
            Where-Object { $_.name -match '(?i)^NETworkManager_.*_Setup\.msi$' } |
            Select-Object -First 1
        if (-not $asset) { throw "No Setup MSI asset found in the latest release." }

        Write-Log "Latest NETworkManager version: $version" -Quiet:$Quiet
        ...
        Write-Log "Failed to get NETworkManager version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageNetworkManager {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NETworkManager (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestNetworkManagerRelease
    if (-not $releaseInfo) { throw "Could not resolve NETworkManager version." }
    ...
    # The MSI ProductVersion drops the release tag's leading century digits, so
    # the detection value is the MSI property and not the tag.
    ...
    # line already installs per-machine.
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "BornToBeRoot" }
...
function Invoke-PackageNetworkManager {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NETworkManager (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestNetworkManagerRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("NETworkManager GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NETworkManager (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageNetworkManager
    }
    elseif ($PackageOnly) {
        Invoke-PackageNetworkManager
    }
    else {
        Invoke-StageNetworkManager
        Invoke-PackageNetworkManager
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-networkmanager'
