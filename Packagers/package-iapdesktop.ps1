...
App: IAP Desktop (x64)
CMName: IAP Desktop
VendorUrl: https://googlecloudplatform.github.io/iap-desktop/
...
.SYNOPSIS
    Packages IAP Desktop (x64) MSI for MECM.

.DESCRIPTION
    Resolves the newest release from the GoogleCloudPlatform/iap-desktop GitHub
    repository, downloads the x64 MSI, stages content to a versioned local
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/GoogleCloudPlatform/iap-desktop/releases?per_page=30"
$AssetName    = "IapDesktopX64.msi"

$VendorFolder = "Google"
$AppFolder    = "IAP Desktop"
$Publisher    = "Google LLC"

$BaseDownloadRoot = Join-Path $DownloadRoot "IapDesktop"
...
function Assert-MsiPayload {
    ...
    .DESCRIPTION
        A CDN that answers 200 with an HTML error body would otherwise stage as a
...
function Get-LatestIapDesktopRelease {
    <#
    .SYNOPSIS
        Returns the newest stable release version and its x64 MSI asset URL.
    .DESCRIPTION
        Release tags are plain version strings. Publication order is not relied
        on; every stable release that carries the x64 MSI is sorted as a version
        instead.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        foreach ($rel in $releases) {
            ...
            if ($rel.tag_name -notmatch '^\d+(\.\d+)+$') { continue }
        ...
        if ($candidates.Count -lt 1) {
            throw "No $AssetName asset found in any stable IAP Desktop release."
        }
        ...
        Write-Log "Failed to get IAP Desktop version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageIapDesktop {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IAP Desktop (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestIapDesktopRelease
    if (-not $releaseInfo) { throw "Could not resolve IAP Desktop version." }
    ...
    # --- Download ---
    $localMsi = Join-Path $BaseDownloadRoot ("{0}-{1}" -f $version, $msiFileName)
    ...
    # The package targets AppDataFolder and declares no ALLUSERS, so an
    # unqualified install lands per-user and is invisible to a device-targeted
    # deployment type; ALLUSERS=1 resolves AppDataFolder to the all-users
    # profile and registers the product in the machine ARP hive.
...
function Invoke-PackageIapDesktop {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IAP Desktop (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestIapDesktopRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("IAP Desktop GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "IAP Desktop (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageIapDesktop
    }
    elseif ($PackageOnly) {
        Invoke-PackageIapDesktop
    }
    else {
        Invoke-StageIapDesktop
        Invoke-PackageIapDesktop
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-iapdesktop'
