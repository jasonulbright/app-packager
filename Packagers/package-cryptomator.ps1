...
App: Cryptomator
CMName: Cryptomator
VendorUrl: https://cryptomator.org/
...
.SYNOPSIS
    Packages Cryptomator (x64) MSI for MECM.

.DESCRIPTION
    Resolves the latest release from the Cryptomator GitHub releases API,
...
.PARAMETER GetLatestVersionOnly
    Outputs only the latest available Cryptomator version string and exits.
...
# --- Configuration ---
$GitHubApiUrl = "https://api.github.com/repos/cryptomator/cryptomator/releases/latest"

$VendorFolder = "Skymatic"
$AppFolder    = "Cryptomator"

$BaseDownloadRoot = Join-Path $DownloadRoot "Cryptomator"
...
function Get-LatestCryptomatorRelease {
    <#
    .SYNOPSIS
        Returns the newest Cryptomator version and its x64 MSI URL.
    .DESCRIPTION
        The release carries Windows, macOS and Linux assets plus a detached
        signature for each, so the asset list is filtered by name rather than
        the tag being trusted to name a downloadable file.
    #>
    param([switch]$Quiet)
    ...
    try {
        ...
        if (-not $m.Success) { throw "Could not locate an x64 MSI asset in the latest release." }

        $version  = $m.Groups['ver'].Value
        $fileName = "Cryptomator-$version-x64.msi"

        Write-Log "Latest Cryptomator version   : $version" -Quiet:$Quiet

        return [pscustomobject]@{
            Version     = $version
            FileName    = $fileName
            DownloadUrl = "https://github.com/cryptomator/cryptomator/releases/download/$version/$fileName"
        }
    }
    catch {
        Write-Log "Failed to get Cryptomator version: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageCryptomator {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Cryptomator (x64) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    Initialize-Folder -Path $BaseDownloadRoot

    $releaseInfo = Get-LatestCryptomatorRelease
    if (-not $releaseInfo) { throw "Could not resolve Cryptomator version." }
    ...
    # The MSI defaults to a per-user install; ALLUSERS=1 forces the per-machine
    # install a device-targeted deployment requires.
    ...
    if ([string]::IsNullOrWhiteSpace($publisher)) { $publisher = "Skymatic GmbH" }
...
function Invoke-PackageCryptomator {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Cryptomator (x64) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
...
if ($GetLatestVersionOnly) {
    try {
        ...
        $info = Get-LatestCryptomatorRelease -Quiet
        if (-not $info) { exit 1 }
        Write-Output $info.Version
        exit 0
    }
    catch {
        [Console]::Error.WriteLine("Cryptomator GetLatestVersionOnly failed: $($_.Exception.Message)")
        exit 1
    }
}
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "Cryptomator (x64) Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if ($StageOnly) {
        Invoke-StageCryptomator
    }
    elseif ($PackageOnly) {
        Invoke-PackageCryptomator
    }
    else {
        Invoke-StageCryptomator
        Invoke-PackageCryptomator
    ...
    Write-LogErrorRecord -ErrorRecord $_ -Context 'package-cryptomator'
